...
if (file.exists(product_path)) {
  ...
  if (length(missing_columns) > 0) {
    cat(paste("The following columns are missing from the products CSV:",
...
if (is.na(gpt_output_file) || gpt_output_file == "") {
  ...
  if (verbose) {
    print(paste("Missing GPT Output File. Setting GPT Output File name to",
                gpt_output_file))
  }
}
...
coicop_labeller <- function(products,
  ...
  update_labelled_df <- function(chunk_index, execution_time = NA) {
    ...
    if (file.exists(gpt_output_file)) {
      ...
      newly_labelled_data <- rbind(labelled_data %>%
